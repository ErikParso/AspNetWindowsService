...
export class AppComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.electronService.isElectronApp) {
      ...
      console.log(this.electronService.remote.process.env.USERNAME, this.electronService.remote.process.env.USERDOMAIN);
      console.log(this.electronService.remote.app.getPath('appData'));
    }

    const startupFile = this.getStartupFile();
    // const startupFile = 'C:\\Users\\eparso\\Desktop\\installXml.hegi';
    const uriScheme = this.getUriScheme();

    if (startupFile && startupFile.endsWith('.hegi')) {
      this.store.dispatch(actions.setStartupFile({ payload: startupFile }));
      if (this.electronService.isElectronApp) {
    ...
    this.store.dispatch(actions.loadVersions());
    this.store.dispatch(actions.loadCurrentUserInfo());
  }
  ...
  showUpgradeInfo() {
    ...
    dialogRef.afterClosed().subscribe((data) => {
      if (data) {
        this.store.dispatch(actions.downloadUpgradePackage());
      }
    });
  }
