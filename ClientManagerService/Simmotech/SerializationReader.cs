...
namespace SimmoTech.Utils.Serialization
{
	...
	/// </summary>
    public /*sealed*/ class SerializationReader : BinaryReader, ASOL.Framework.Shared.Serialization.ISerializationReader // VS 0001532 sealed removed
	{
        ...
        //HEB+ 20061121 new constructor added
        public SerializationReader(byte[] data, int index, int count) : this(new MemoryStream(data, index, count)) { }
        //HEB-
		...
		internal BitVector32 ReadBitVector32()
		{
        ...
        {
#if PORTABLE
            throw new NotImplementedException("SerializationReader: DateTimeOffset is not supported for PORTABLE at a time.");
#else // NETFW or NETCORE
            var dt = DateTimeOffset.FromUnixTimeMilliseconds(ReadInt64());
            return dt;
#endif

            //var dt = DateTime.FromBinary(ReadInt64()); 
		...
		/// </summary>
		/// <returns>A BitVector32 value.</returns>
        internal BitVector32 ReadOptimizedBitVector32()
		{
        ...
        {
            //JR 0002935 - pro value typy, které májí svoji přímou metodu pro vrácení pole, je použita tato přímá metoda
            var type = typeof (T);
            if (type.IsValueType)
            {
                ...
                switch (typeCode)
                {
                    ...
                    case TypeCode.Boolean:
                        return (T[]) (object) ReadBooleanArray();
                    case TypeCode.Char:
                        return (T[]) (object) ReadCharArray();
                    case TypeCode.DateTime:
                        return (T[]) (object) ReadDateTimeArray();
                    case TypeCode.Decimal:
                        return (T[]) (object) ReadDecimalArray();
                    case TypeCode.Double:
                        return (T[]) (object) ReadDoubleArray();
                    case TypeCode.Int16:
                        return (T[]) (object) ReadInt16Array();
                    case TypeCode.Int32:
                        return (T[]) (object) ReadInt32Array();
                    case TypeCode.Int64:
                        return (T[]) (object) ReadInt64Array();
                    case TypeCode.SByte:
                        return (T[]) (object) ReadSByteArray();
                    case TypeCode.Single:
                        return (T[]) (object) ReadSingleArray();
                    case TypeCode.String:
                        return (T[]) (object) ReadStringArray();
                    case TypeCode.UInt16:
                        return (T[]) (object) ReadUInt16Array();
                    case TypeCode.UInt32:
                        return (T[]) (object) ReadUInt32Array();
                    case TypeCode.UInt64:
                        return (T[]) (object) ReadUInt64Array();
                    case TypeCode.Object:
                        if (type == typeof (Guid))
                            return (T[]) (object) ReadGuidArray();
                        else if (type == typeof (TimeSpan))
                            return (T[]) (object) ReadTimeSpanArray();
                        break;
                }
            }
            return (T[])ProcessArrayTypes(ReadTypeCode(), null);
        }
        ...
        private Dictionary<K, V> _ReadNotNullDictionary<K, V>() // VS
        {
            var result = new Dictionary<K, V>();
            _ReadNotNullDictionary(result);
            return result;
        }

        private Dictionary<K, V> _ReadNullableDictionary<K, V>() // VS 0001532
        {
            bool isNull = ReadBoolean();
            if (isNull) return null;
            return _ReadNotNullDictionary<K, V>();
        }
        ...
        public Dictionary<K, V> ReadDictionary<K, V>()
        {
            ...
            return _ReadNullableDictionary<K, V>(); // VS 0001532
        }

        private void _ReadNotNullDictionary<K, V>(Dictionary<K, V> dictionary) // VS 0001532
        {
        ...
        /// <typeparam name="K">The key Type.</typeparam>
        /// <typeparam name="V">The value Type.</typeparam>
        public void ReadDictionary<K, V>(Dictionary<K, V> dictionary) // VS 0001532
        {
            // VS 0001532 (REM)
            //var keys = (K[]) ProcessArrayTypes(ReadTypeCode(), typeof(K));
            //var values = (V[]) ProcessArrayTypes(ReadTypeCode(), typeof(V));
            ...
            _ReadNotNullDictionary(dictionary); // VS 0001532
        }
        #endregion
		...
		public List<T> ReadList<T>()
		{
		    ...
		    return _ReadNullableList<T>(); // VS
		}

        private List<T> _ReadNullableList<T>()
        {
            bool isNull = ReadBoolean();
            if (isNull) return null;
            return _ReadNotNullList<T>();
        }

        private List<T> _ReadNotNullList<T>() // VS
        {
            return new List<T>((T[])ProcessArrayTypes(ReadTypeCode(), typeof(T))); // pùvodní implementace ReadList<T>
        }
        ...
        public DateTimeOffset[] ReadDateTimeOffsetArray()
        {
            ...
            {
                case SerializedType.NullType: return null;
                case SerializedType.EmptyTypedArrayType: return new DateTimeOffset[0];

                default:
                    var optimizeFlags = ReadTypedArrayOptimizeFlags(typeCode);
                    var result = new DateTimeOffset[ReadOptimizedInt32()];
                    ...
                    for (var i = 0; i < result.Length; i++)
                    {
                        ...
                        {
                            // result[i] = ReadOptimizedDateTime();
                            throw new NotSupportedException("ReadOptimizedDateTime call in 'public DateTimeOffset[] ReadDateTimeOffsetArray()'");
                        }
                    }
        ...
        public void ReadOwnedData(ISelfSerializable target, object context)
		{
			target.Deserialize(this, context);
		}
		...
		object ProcessObject(SerializedType typeCode)
		{
			...
			switch (typeCode)
			{
                ...
                case SerializedType.DateTimeOffsetType: return ReadDateTimeOffset();
                ...
                case SerializedType.MinDateTimeOffsetType: return DateTimeOffset.MinValue;
                ...
                case SerializedType.MaxDateTimeOffsetType: return DateTimeOffset.MaxValue;  
				...
				case SerializedType.OtherType:
                    //HEB replace standard formatter  return new BinaryFormatter().Deserialize(BaseStream);
                    return DeserializeUnsupportedObject();
				...
				case SerializedType.OwnedDataSerializableAndRecreatableType:
			        {
			            ...
			            {
			                throw new InvalidOperationException(string.Format("An instance of type '{0}' could not be created. See inner exception for details.", t), e);
			            }
			            ReadOwnedData((ISelfSerializable)result, null);
				...
				case SerializedType.SurrogateHandledType:
					{
                        ...
                        // var typeSurrogate = SerializationWriter.FindSurrogateForType(serializedType); // VS 0001532 (REM)
                        var typeSurrogate = FindSurrogateForType(serializedType); // VS 0001532 instance method used to allow overriding
        ...
        ///// </summary>
        ///// <returns>An Enum value.</returns>
        //public T ReadEnum<T>() where T : struct // VS 0001532 (extracted from ReadObject method)
        //{
        //    var typeCode = (SerializedType)ReadByte();
        //    switch(typeCode)
        //    {
        //        case SerializedType.OptimizedEnumType:
        //            {
        ...
        //                    (underlyingType == typeof(long)) || (underlyingType == typeof(ulong)))
        //                {
        //                    return (T)Enum.ToObject(enumType, ReadOptimizedUInt64());
        //                }
        ...
        //                if (underlyingType == typeof(Int32)) return (T)Enum.ToObject(enumType, ReadInt32());
        //                if (underlyingType == typeof(Byte)) return (T)Enum.ToObject(enumType, ReadByte());
        //                if (underlyingType == typeof(Int16)) return (T)Enum.ToObject(enumType, ReadInt16());
        //                if (underlyingType == typeof(UInt32)) return (T)Enum.ToObject(enumType, ReadUInt32());
        //                if (underlyingType == typeof(Int64)) return (T)Enum.ToObject(enumType, ReadInt64());
        //                if (underlyingType == typeof(SByte)) return (T)Enum.ToObject(enumType, ReadSByte());
        //                if (underlyingType == typeof(UInt16)) return (T)Enum.ToObject(enumType, ReadUInt16());

        //                return (T)Enum.ToObject(enumType, ReadUInt64());
        //            }
        //        default:
        //            throw ASOL.Framework.Shared.ThrowHelper.InvalidEnumValue(typeCode);
        //    }
        //}
        ...
        /// <returns></returns>
        protected virtual /*static*/ IFastSerializationTypeSurrogate FindSurrogateForType(Type type) // VS 0001532 static to virtual
        {
            foreach (var surrogate in SerializationWriter.TypeSurrogates)
            {
        ...
        protected virtual object DeserializeUnsupportedObject()
        {
#if NETFW
            return new BinaryFormatter().Deserialize(BaseStream);
#else
            throw new NotSupportedException("Deserialization of unsupported type isn't supported.");
#endif
        }
        #endregion
		...
		/// <returns></returns>
		public object ProcessArrayTypes(SerializedType typeCode, Type defaultElementType)
		{
			switch (typeCode)
			{
                ...
                case SerializedType.DateTimeOffsetArrayType: return ReadDateTimeOffsetArray();
				...
				case SerializedType.FullyOptimizedTypedArrayType:
					...
					for (var i = 0; i < length; i++)
					{
						...
						{
                            var value = (ISelfSerializable)Activator.CreateInstance(defaultElementType);
		...
		public SerializedType ReadTypeCode()
		{
