...
using System.Linq;
using ASOL.Framework.Shared;
using ASOL.Framework.Shared.Serialization; // VS 0001532 ISelfSerializable interface used instead of IOwnedDataSerializableAndRecreatable / IOwnedDataSerializable.
...
namespace SimmoTech.Utils.Serialization
{
	...
	/// </code></example>
	/// </summary>
    public /*sealed*/ class SerializationWriter : BinaryWriter, ASOL.Framework.Shared.Serialization.ISerializationWriter // VS0001532 sealed removed
    {
		...
		internal void Write(BitVector32 value)
		{
			Write(value.Data);
		}
        ...
        {
#if PORTABLE
            throw new NotImplementedException("SerializationWriter: DateTimeOffset is not supported for PORTABLE at a time.");
#else // NETFW or NETCORE
		...
		/// <summary>
        /// Allows any object implementing ISelfSerializable to serialize itself
		...
		/// </summary>
        /// <param name="target">The ISelfSerializable object to ask for owned data</param>
		/// <param name="context">An arbtritrary object but BitVector32 recommended</param>
        public void Write(ISelfSerializable target, object context)
		{
			target.Serialize(this, context);
		}
        ...
        public void Write(Enum enumValue) 
        {
            var value = (object)enumValue;
        ...
        /// </summary>
        /// <param name="enumValue"></param>
        public void WriteEnumAsObject(Enum enumValue) // VS 0001532 (extracted from Write(object value) method)
        {
            var value = (object)enumValue;
            var enumType = value.GetType();
            var underlyingType = Enum.GetUnderlyingType(enumType);

            switch (System.Type.GetTypeCode(underlyingType))
            {
                case TypeCode.Int32:
        ...
        public void WriteObject(object value)
		{
            ...
            else if (IsTypeRecreatable(value.GetType()))
            {
                ...
                //Write((ISelfSerializable)value, null);

                _WriteSelfSerializable(value); // VS 0001532
            }
            ...
            else if (value is DateTimeOffset)
            {
                ...
                if (dateTimeValue == DateTimeOffset.MinValue)
                {
                    WriteTypeCode(SerializedType.MinDateTimeOffsetType);
                }
                else if (dateTimeValue == DateTimeOffset.MaxValue)
                {
                    WriteTypeCode(SerializedType.MaxDateTimeOffsetType);
                }
                else
                {
                    WriteTypeCode(SerializedType.DateTimeOffsetType);
                    Write(dateTimeValue);
                }
            }
            ...
            // VS 0001532 (REM) Moved above to increase performance for ISelfSerializable
            //else if (IsTypeRecreatable(value.GetType()))
            //{
            //    WriteTypeCode(SerializedType.OwnedDataSerializableAndRecreatableType);
            //    WriteOptimized(value.GetType());
            ...
            //HEB+ 20061122 ignores possible descendants of ArrayList
            else if (value is ArrayList)
            {
                bool exact = (value.GetType() == typeof(ArrayList));
                if (exact)
                {
                    WriteTypeCode(SerializedType.ArrayListType);
                ...
                {	//HEB otherwise use standard serialization
                    WriteTypeCode(SerializedType.OtherType);
                    //HEB replace standard formatter  createBinaryFormatter().Serialize(BaseStream, value);
                    SerializeUnsupportedObject(value);
                }
            }
            //HEB-
			...
			{
                WriteEnumAsObject((Enum)value); // VS 0001532 kód zde v této vìtvi vyèlenìn do metody Write(Enum value).
			}
			else
			{
        ...
        /// <summary>
        /// Writes a ISelfSerializable object into the stream.
        /// </summary>
        /// <param name="value">The ISelfSerializable value to store.</param>
        public void WriteObject(ISelfSerializable value) // VS 0001532  - aby se nemusely procházet if else-if vìtve, když mám objekt, který implementuje ISelfSerializable
        {
            if (value == null)
            {
                WriteTypeCode(SerializedType.NullType);
            ...
            {
                _WriteSelfSerializable(value);
            }
        }

        private void _WriteSelfSerializable(object value) // VS 0001532
        {
	        WriteTypeCode(SerializedType.OwnedDataSerializableAndRecreatableType);
	        WriteOptimized(value.GetType());
	        Write((ISelfSerializable)value, null);
        ...
        public void Write(Type value) // VS 0001532
        {
            Write(value, true);
        }
		...
		public void Write(Type value, bool fullyQualified)
		{
			...
			else
			{
                ...
                string s = GetStringIdentifierForType(value);
                WriteOptimized(s);
			}
		}
		...
		public void WriteOptimized(BitArray value)
		{
			...
			if (value.Length > 0)
			{
				...
				((ICollection)value).CopyTo(data, 0);
        ...
        internal void WriteOptimized(BitVector32 value)
		{
		...
		public void WriteOptimized(Type value)
		{
            ...
            //         // VS 0001532 (REM): tohle je špatnì, pokud je pøedán generický typ, kde je jako T typ z mscorlib. 
            //         //                   Napø. typ 'ASOL.Framework.Shared.DataSynchronization.DataContracts.DataTypes.DataTableOtherChangesExchangeItem`1' from assembly 'ASOL.Framework.Shared, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null'  
            //// WriteOptimized(value.AssemblyQualifiedName.IndexOf(", mscorlib,") == -1 ? value.AssemblyQualifiedName : value.FullName);

            //         WriteOptimized(value.GetAssembly().FullName.StartsWith("mscorlib,") ? value.FullName : value.AssemblyQualifiedName); // VS 0001532 (oprava pùvodního kódu)
            string s = GetStringIdentifierForType(value);
            WriteOptimized(s);
        }

        internal static string GetStringIdentifierForType(Type type)
        {
            //return ClrType.GetTypeQualifiedName(type);
            return _GetTypeName(type, true);
        }

        private static string _GetTypeName(Type type, bool includeAssembly) // VS
        {
            StringBuilder typeIdentifier = new StringBuilder();
            if (type.IsGenericType)
            {
                var typedef = type.GetGenericTypeDefinition();
                typeIdentifier.Append(typedef.FullName);
                ...
                foreach (var genericArgument in genericArguments)
                {
                    if (!first)
                        typeIdentifier.Append(",");
                    typeIdentifier.Append("[");
                    string identifier = _GetTypeName(genericArgument, true);
                    typeIdentifier.Append(identifier);
                    typeIdentifier.Append("]");
                    first = false;
                }
                typeIdentifier.Append("]");
            ...
            {
                typeIdentifier.Append(type.FullName);
            }
            ...
            if (includeAssembly)
            {
                ...
                {
                    // "System" assembly is the only exception - e.g. in UWP, it is not allowed to use Name, FullName must be used
                    typeIdentifier.Append(assemblyName.FullName);
                }
                else
                {
                    typeIdentifier.Append(type.Assembly.GetName().Name); // no need to use Assembly fullname in this
                }
            }
            string s = typeIdentifier.ToString();
            return s;
        }
        ...
        /// </summary>
        /// <param name="values">The DateTime[] to store.</param>
        public void Write(DateTimeOffset[] values)
        {
            if (values == null)
            {
                WriteTypeCode(SerializedType.NullType);
        ...
        #region Dictionary // VS 0001532

        // VS + MAF 0001532 Dictionary je normální reference typ, tedy volání metody Write(Dictionary dict) musí podporovat null na vstupu.
        ...
        public void Write<K, V>(Dictionary<K, V> value)
        {
            ...
            _WriteNullableDictionary(value); // VS 0001532
        }

        private void _WriteNotNullDictionary<K, V>(Dictionary<K, V> value) // VS 0001532
        {
            var keys = new K[value.Count];
            value.Keys.CopyTo(keys, 0);
        ...
        private void _WriteNullableDictionary<K, V>(Dictionary<K, V> value) // VS 0001532
        {
            Write(value == null);
            if (value == null) return;
            _WriteNotNullDictionary(value);
        }

        #endregion

        #region List // VS 0001532

        // VS + MAF 0001532 List je normální reference typ, tedy volání metody Write(Dictionary dict) musí podporovat null na vstupu.
		...
		{
            // WriteTypedArray(value.ToArray(), false); // VS 0001532 (REM)
            _WriteNullableList(value); // VS 0001532
		}

        private void _WriteNullableList<T>(List<T> value) // VS 0001532
        {
            Write(value == null);
            if (value == null) return;
            _WriteNotNullList(value);
        }

        private void _WriteNotNullList<T>(List<T> value) // VS 0001532
        {
        ...
        public void Write<T>(T[] values)
        {
            ...
            {
                WriteTypedArray(values, true, typeof(T));
            }
        }
        ...
        #region Protected virtual methods //HEB
        protected virtual void SerializeUnsupportedObject(object value)
        {
#if NETFW
            CreateBinaryFormatter().Serialize(BaseStream, value);
#else
            throw new NotSupportedException(String.Format("Serialization of type '{0}' isn't supported.", value != null ? value.GetType().FullName : "null"));
#endif
        }
        #endregion
		...
		/// <returns></returns>
		protected virtual /*static*/ IFastSerializationTypeSurrogate FindSurrogateForType(Type type) 
		{
        ...
        /// a reference to constant FullyOptimizableValueArray if all the elements are optimizable; or null
        /// if none of the elements are optimizable.</param>
        void WriteArray(DateTimeOffset[] values, BitArray optimizeFlags)
        {
            WriteTypedArrayTypeCode(optimizeFlags, values.Length);
            ...
            for (var i = 0; i < values.Length; i++)
            {
                ...
                {
                    // WriteOptimized(values[i]);
                    throw new NotSupportedException("WriteOptimized call in 'void WriteArray(DateTimeOffset[] values, BitArray optimizeFlags)'");
                }
            }
        }
        ...
        /*private*/
        protected void WriteTypeCode(SerializedType typeCode) // VS 0001532 private -> protected
		{
			Write((byte) typeCode);
#if DEBUG
        ...
        public void WriteTypedArray(Array value, bool storeType)
        {
            var elementType = value.GetType().GetElementType();
            WriteTypedArray(value, storeType, elementType);
        }

        private void WriteTypedArray(Array value, bool storeType, Type elementType)
		{
            ...
            {
                WriteTypeCode(SerializedType.DateTimeOffsetArrayType);
			...
			else
			{
				...
				for (var i = 0; i < value.Length; i++)
				{
					...
					{
                        Write((ISelfSerializable)value.GetValue(i), null);
					}
				}
			}
		}
		...
		{
            // VS 0001532 (REM): Zjednodušení metody IsTypeRecreatable(Type type). Pokud tøída implementuje ISelfSerializable, tak musí obsahovat public konstruktor, aby jí bylo možné bezpeènì deserializovat.
			//if (type.IsValueType) return typeof(ISelfSerializable).IsAssignableFrom(type);
            // return typeof(IOwnedDataSerializableAndRecreatable).IsAssignableFrom(type) && HasEmptyConstructor(type); 

            return typeof(ISelfSerializable).IsAssignableFrom(type);
		}
...
#else
            return type.GetTypeInfo().DeclaredConstructors.Any(c => c.IsConstructor && c.GetParameters().Length == 0);
#endif
		}
